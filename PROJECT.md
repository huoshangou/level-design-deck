...
| 2026-05-11 | M3.11 + M3.12 启动 · A 阶段第 3+4 个 module = `vfx_req` + `audio_req`（克隆型双发） | 决定单 sonnet 顺序做两个（不并发），避免 cross_check.py / generate_spec.py / template_diff.py 等共用文件冲突。两个 module 都触发 cross_check 新规则（zone_id ref spatial label），是第 3、4 条规则。`[来源: Steve 直接指示（2026-05-11）+ 第一原理推导]` |
| 2026-05-11 | M3.11 vfx_req schema 设计 · 不含玩法技能特效 | 范围 = 环境特效 + 可交互物件特效 + 整体氛围 + 剧情节点。**不含玩法技能特效**（玩家技能/受击是数值组的事，不是 POI 的事）。category enum: environmental / interactive_object / ambient / narrative。`[来源: 第一原理推导]` |
| 2026-05-11 | M3.12 audio_req schema 设计 · 参考 work_docs 环境音字段 | ambient_sounds 字段直接对照 work_docs "环境音需求_室内外" 9 字段（区域/时间段/声音描述/天气/音源/可听范围/频率/状态等）。music 字段 PoC 期保留（场景配乐如 Part1 潜入 / Part2 Boss 战）。`[来源: work_docs_extract.json + 第一原理推导]` |
| 2026-05-11 | M3.11+M3.12 关键架构 · cross_check.py 二次重构 helper | sonnet 主动提取 `_check_zone_field(v, spatial_labels, items, field_path_prefix, id_key)` helper，4 个 zone_ref 校验函数都改成一行复用。cross_check.py 271→286 < 300 ✓。"加新 zone_ref 规则只需 1 行 register + 4 行函数体"成立。`[来源: 第一原理推导 + sonnet 自主决策]` |
| 2026-05-11 | M3.11+M3.12 ✅ 完成 | 2 schema 新建（vfx 74 / audio 83）+ 2 spec 真实数据（vfx 5 effects 含「障子散射光晕/香火烟雾/冷月光光斑/竹叶飘落/烟雾弹」专业术语，4/5 命中 spatial label；audio 4 ambient_sounds + 2 music「Part1 极简沉浸/低频琵琶尺八」「Part2 高压太鼓/金属打击」）+ 2 template 新建（vfx 66 / audio 80 卡片式）+ cross_check.py 加 2 规则 + helper 二次重构（286 行）+ template_diff 加 2 跳过 + generate/regenerate MODULES 注册。**验证全过**：vfx 0/0 / audio 0/0 / cross_check 7 modules 4 rules 0 ERROR / 2 个故意破坏全部 ERROR cross_ref_integrity / server check+render 全 ok / 5 原 module 回归全 0 ✓。**反污染自检**：sonnet 自报无触发。`[来源: Steve 直接指示（2026-05-11）+ 第一原理推导]` |
| 2026-05-11 | M3.12.1 hotfix · cross_check.py --specs 模式 isolation 修复 | bug：M3.12 后 Steve 实测发现 ghost_audio_zone ERROR 没消，原因是 sonnet 测试故意破坏时 cross_check.py `--specs` 模式默认写到正式 outputs/.cross_warnings.json，污染浏览器 alerts 显示。fix：`--output` 默认 None，按模式决定路径：--level-id → 正式持久化 / --specs → /tmp/.cross_warnings_specs.json（隔离）+ 修 print relative_to /tmp 路径崩 ValueError 副作用。**测试流程纪律**：未来委托 sonnet 跑 cross_check 故意破坏测试时，工具默认就隔离，不靠 sonnet 自觉。`[来源: Steve 直接指示（2026-05-11）+ 第一原理推导]` |
| 2026-05-11 | M3.13 启动 · A 阶段第 5 个 / 收尾 module = `asset_list` | 给制作团队的对接表（资产名/类别/引用区域/状态）。**关键约束**（user MEMORY `feedback_no_made_up_asset_naming`）：asset_id 字段公司命名待对接，PoC 期 deck 绝不能编造伪接口名（如 `model_xxx_001`），一律 `[待对接]` 占位。是 cross_check 第 5 条规则触发。`[来源: Steve 直接指示（2026-05-11）+ user MEMORY + 第一原理推导]` |
| 2026-05-11 | M3.13 schema 设计 · category 8 enum + asset_id 反污染约束 | category enum: model_3d / texture / vfx / audio / animation / interactive_object / lighting / other（覆盖 POI 常见 8 类）。asset_id 字段 description 显式写"禁止编造伪接口名（如 model_xxx_001）"，PoC 期一律 `[待对接]` 占位。`[来源: 第一原理推导 + user MEMORY 反污染约束]` |
| 2026-05-13 | M3.13 ✅ 完成 | 1 schema 新建 85 行 + 1 spec 新建（11 资产覆盖 6 类：3 model_3d「白狐神龛/障子拉门/竹林雕像」+ 2 texture「障子纸半透/木地板」+ 2 vfx「香火烟雾/冷月光斑」+ 2 interactive_object「Key 道具组/白狐暗门」+ 1 lighting「石头灯笼」+ 1 audio「夜间环境音」，**全部 11 个 asset_id = "[待对接]" 0 伪接口**）+ 1 template 新建 154 行（表格式适合对接清单）+ cross_check.py 加第 5 条规则（299 行 < 300 严守）+ template_diff 加 asset_list_ 跳过 + generate/regenerate MODULES 注册。**12 项测试全过 + 反污染自检脚本输出"✓ 所有 asset_id 都留 [待对接] 占位"**。`[来源: Steve 直接指示（2026-05-11）+ 第一原理推导]` |
| 2026-05-13 | **A 阶段完结** · 8 module + 5 cross_check 规则 + UX 层 + LevelCraft 集成 + spec 真源生态闭环 | 同 level（gangster_mansion）现有 8 个 spec：spatial_layout / bubble_diagram / lighting_req / level_overview / atmosphere_ref / vfx_req / audio_req / asset_list。cross_check 5 规则全过（lighting/vfx/audio/atmosphere zone_id ⊆ spatial label + asset_list ref_zone_id ⊆ spatial label）。**deck 范式相对零散文档的核心优势已验证**：(1) spec.json 真源 + AI 友好；(2) schema-driven UI + 人话 title 双层分离；(3) 机械检测优先 + cross_check 跨 spec 联动校验是 word/wiki 完全做不到的。**下一阶段**：壳前准备（user 终极目标 = "类似 app 的壳让不会 cc 的人也能用"）/ 或继续 module 补全（emotion_curve/tech_req/storyboard 按需驱动）/ 或 P2 alerts 人话化。待 Steve 选定。`[来源: 全程一致执行 + Steve 终极目标]` |
| 2026-05-12 | M3.13.1 · 完整关卡文档渲染（同 level 多 module 拼接）| 新工具 tools/render_level.py（185 行）按 --level-id 把同 level 8 module HTML 用 iframe + sticky nav 拼成一份"完整关卡文档"。serve_editor 加 /api/render-level 端点 + editor.html 加「📚 完整关卡文档」按钮（onclick 函数挪到 editor/views/level_doc.js 兑现"超 900 拆 views/"决策）。**用户体验**：浏览器点按钮 → server 自动 render-missing → 跳转完整文档；不会 cc 的设计师无需碰终端。`[来源: Steve 直接指示（2026-05-11）]` |
| 2026-05-12 | **M3.14 · 壳前基础设施**（终极目标"app 壳"铺路）| Steve 直接指示走 Tauri/Electron/一键脚本/内网部署 4 选项之前先做基础。**3 件事**：(a) Mermaid 本地化（lib/mermaid.min.js 3.2MB，消除唯一外网 CDN 依赖）；(b) 统一 lib/ 目录（未来字体/图标/Three.js 都放这）；(c) start.command 一键启动脚本（双击启动 server + 自动开浏览器，不会 cc 的设计师立刻能用）。**Mermaid 本地化** = editor.html + templates/bubble_diagram.html.tmpl 两处 src 改 ../lib/mermaid.min.js。**Three.js 本地化暂留 TODO**（仅 spatial_layout 用，importmap addons 目录复杂，单独里程碑做）。`[来源: Steve 直接指示（2026-05-12）+ 第一原理推导]` |
...
| 2026-05-25 | **M4.1 启动** · 第 9 个 module = `storyboard`（key art × 玩法 beat）| **缺位诊断**：bubble_diagram 有 beat 无画面（纯节点+边的抽象逻辑），atmosphere_ref 有画面按空间分（zone 维度），缺时间维度上的画面层。**产出物边界**：本 module 产出物 = img2img 文字提示词（`generated_prompt`），不是给设计师读的文档；spec 顶部 `style_anchor` 全局共享 + Python deterministic 拼接，保证「改一处 anchor → 全 panel 同步」。当前无 image-2 API，先做 prompt 字符串 + 留 API 接口预留。**风格定调**：日式 imageboard / 絵コンテ 铅笔+墨线（Steve 提供赛车 imageboard 参考图反推）。**首批案例** = abandoned_temple（已有 8 module 完整前置数据，0 阻塞）。**milestone 拆分** = M4.1（PoC 端到端 schema/spec/template/cross_check）+ M4.2（拼接器 + 工具链集成）。`[来源: Steve 直接指示（2026-05-25）+ 第一原理推导]` |
| 2026-05-25 | M4.1 schema 决策 · style_anchor 塞 spec 顶部 + panels[] 双字段层 | style_anchor 顶部 self-contained（不抽出 style_bible 新 module），含 medium/art_direction/color_palette/lighting_aesthetic/quality_tags/negative_prompt/reference_image_url/presentation 8 段；panels[] 字段分两层：**prompt 原料**（scene/subject_action/camera/lighting/mood，英文，入 generated_prompt）+ **设计意图**（player_intent/player_feels/gameplay_moment，中文，仅供人读 + cross_check）。空字段继承：`panel.lighting` 空 → 拼接时继承 `style_anchor.lighting_aesthetic`；`panel.source_image_url` 空 → 继承 `style_anchor.reference_image_url`。装帧（米色纸 + 黑框）由 CSS 做，不让 AI 生成（一致性该交给 deterministic 层）。`[来源: Steve 直接指示（2026-05-25）+ 第一原理推导]` |
| 2026-05-25 | M4.1 cross_check 第 7 条规则 · `storyboard.panels[].beat_id ∈ bubble_diagram.nodes[].id` | 新独立装饰器函数 `check_storyboard_beat_ref`，独立于现有 5 条 zone_ref 规则。同时 `_ZONE_REF_RULES` 列表追加 1 项 `storyboard.panels[].zone_id`（沿用机制）。**故意破坏验证**：beat_id='nonexistent_beat' → 精确报 ERROR；zone_id='幻影地' → 精确报 zone_ref ERROR。`[来源: 第一原理推导]` |
| 2026-05-25 | M4.1 ✅ 完成 | **新建**：schema/storyboard.schema.json（177 行）+ specs/storyboard_abandoned_temple.spec.json（5 panel，跨 Part 1/2，覆盖 entry/choice/combat/scene/combat beat type）+ templates/storyboard.html.tmpl（240 行，含 CSS 装帧米色纸 + 黑框 + 16:9）。**改**：tools/cross_check.py 286→304 行（**⚠️ 软超 300 硬限 4 行**；已记 P2 候选 = 拆 `_cross_check_helpers.py`）。**验证**：storyboard 0/0；cross_check 9 modules / 9 checks / 0 ERROR / 1 已有 REVIEW；故意破坏 2 ERROR 准确命中 + 恢复后归零；render HTML 20.5KB；8 个原 module 回归全 0 ERROR。`[来源: Steve 直接指示（2026-05-25）]` |
| 2026-05-25 | **M4.2 启动** · prompt 拼接器 + ImageProvider API 接口预留 + 全工具链集成 | 新工具 `tools/storyboard_render.py`：PromptComposer 类（按 prompt_template + style_anchor + panel 字段 deterministic 拼接，正则替换 `{style.X}` 和 `{X}`，空字段自动继承，连续逗号去重）+ ImageProvider ABC + DummyProvider 实现 + 原子写回 spec。当前 CLI：`--prompt-only` 跑通；未来加 `--provider image2 --api-key $K` 子类即可。**集成 5 处**：generate_spec.py MODULES 注册 / render_level.py MODULE_ORDER+LABELS / render_deck.py MODULE_ORDER+META / template_diff.py SKIP_PREFIXES / cc-skills/design-deck.md 推荐流程。`[来源: Steve 直接指示（2026-05-25）]` |
| 2026-05-25 | M4.2 prompt 拼接发现冗余 · 删 default template 末尾 `{style.lighting_aesthetic}` | 因为 `panel.lighting` 空时已经继承 `style.lighting_aesthetic`，模板末尾再带一次造成 prompt 字符串冗余。修正：default prompt_template 删末尾 `{style.lighting_aesthetic}`（3 处同步：schema default / abandoned_temple spec / storyboard_render.py DEFAULT_TEMPLATE）。验证：p01（lighting 空）继承 1 次 ✓；p03（lighting 手填）覆盖 1 次 ✓。`[来源: 第一原理推导]` |
| 2026-05-25 | M4.2 ✅ 完成 | **新建**：tools/storyboard_render.py（230 行 < 300 ✓）。**改 5 处**：generate_spec.py +8 行 / render_level.py +2 行（MODULE_ORDER 第 4 位插 storyboard + LABELS 加「关卡分镜」）/ render_deck.py +2 行（MODULE_ORDER 第 3 位插 + META 加「关卡分镜/Storyboard」）/ template_diff.py +1 行（SKIP_PREFIXES `storyboard_`）/ cc-skills/design-deck.md（推荐流程 + 已支持 module 列表 + 顺序理由更新）。**端到端验证**：storyboard_render 5 prompts composed ✓；generate_spec --module storyboard 出 486 行 prompt ✓；template_diff skipped ✓；render_level 9 modules → full.html ✓；render_deck 158 行 deck ✓；8 module 回归全 0 ✓。`[来源: Steve 直接指示（2026-05-25）]` |
| 2026-05-25 | M4.3 拆 `tools/_cross_check_helpers.py` · 偿还 M4.1 技术债 | M4.1 触发：cross_check.py 加 storyboard 第 7 条规则后 286→304 行，软超 300 硬限 4 行，阻塞后续规则。**方案 B**：helper 文件放通用工具（`get_spatial_labels` / `check_zone_field` / `make_zone_ref_check` 工厂），主文件保留 `_ZONE_REF_RULES` 列表 + 注册 + 3 个独立规则（让"哪些 module 做 zone ref"的决策一眼可见）。**结果**：cross_check.py 304→272（腾出 28 行预算）+ _cross_check_helpers.py 65 行。**回归**：9 module / 9 checks / 0 ERROR / 1 已有 REVIEW ✓；2 故意破坏精确命中 + 恢复归零 ✓；9 module mechanical_check 全 0 ✓；外部 import `tools._cross_check_helpers` 通 ✓（webapp/backend 复用无障碍）。`[来源: 第一原理推导 + M4.1 触发]` |
| 2026-05-26 | **fill-gamedoc skill 加 3 条接力修改纪律 + ai-review-box 升级为折叠 chip** | session 8bf0befd（动态货运卡车文档）暴露 3 个真实 bug：(1) cc 给"新方案推翻旧方案"时按字段 checklist 改必漏，自报"24✅"实际还有 4-5 处旧术语（30m/半接管/双段/`gameplay_name_en` 副标题等）零散散布在 design_goal/uiux_requirement/sfx_requirement 各字段；(2) 用户报告"没看到改动"时 cc 第一反应甩锅给浏览器缓存（L262/L307），实际是文件本身没改干净；(3) mermaid 字段 Edit 局部替换残留孤立行 `B --> C{成功条件}` → 整张图渲染失败。**修复**：cc-skills/fill-gamedoc.md 加 Phase 4.4（接力修改 checklist：旧→新术语映射表 + Grep 验证完工判据 + 白名单）/ 4.5（diff 反馈反射：禁止先归因缓存，必须先 Grep 验证文件状态）/ 4.6（mermaid 字段编辑硬规则：只允许整段 `<div class="mermaid">...</div>` 替换）；并把 ai-review-box 模板从 sticky div 改成默认折叠的 `<details>` chip（解决奶油色块挤占预览栏）。webapp 端补 chat.py `_active_ws` ownership 校验（修 StrictMode race 导致的 409）+ PreviewPane 已知缺刷新按钮（P2 候选）。skill 文件 484→512 行；symlink `~/.claude/commands/fill-gamedoc.md` 自动同步。`[来源: Steve 直接指示（2026-05-26）+ session 8bf0befd 3 bug 复盘]` |
| 2026-05-27 | **借鉴 projectx-leveldesign：DocSkeleton 派生器 + Stop Hook 自动落盘** | 同事仓库 projectx-leveldesign 两个设计治 LDD fill-gamedoc 痛点：(1) cc 接力修改时不看文档当前状态就动手 → 漏字段、改错地方反复；(2) 长对话 [决策]/[待定] 标记不沉淀，下次接手得人工读 transcript。**新建**：`tools/doc_skeleton.py`（282 行，stdlib only，从 reference/template_fields.json + HTML 解析派生骨架：每字段当前值 + ❗待填 + ai-flag 列表 + checkbox 状态）；`tools/stop_hook.py`（215 行，stdlib only，cc Stop 事件扫 transcript 抽 5 种设计标记追加到 memory/pending_review.md，silent fail 不阻塞 cc）；`.claude/settings.json`（注册 Stop hook 用 `$CLAUDE_PROJECT_DIR` 跨平台变量）；`memory/.gitkeep`（目录占位）。**改**：fill-gamedoc.md 加 Phase 4.0「修改前必读骨架」强制 + 3.6 完工 gate 加 doc_skeleton.py 对账；CLAUDE.md 加第 8 节 Session 启动流程（原 8 节顺延 9）v0.8→v0.9。**验证**：doc_skeleton 跑卡车文档输出 37 字段 / 32 ✓ / 5 ❗待填 / mermaid 28 行 12 节点；stop_hook 模拟 transcript 含 [决策][待定][风险] 标记追加成功，代码块假阳性正确剥离；symlink ~/.claude/commands/fill-gamedoc.md 自动同步。**不破坏 LDD 哲学**：HTML 直产 + 单机 cc CLI 可跑都保留，纯加成。`[来源: Steve 直接指示（2026-05-27）+ projectx-leveldesign 借鉴]` |
| 2026-06-04 | **storyboard schema v0.7→v0.8 · prompt 质量系统** | **问题**：(1) venue_type 污染非本场景 panel（俱乐部描述出现在车内）；(2) 多角色外观逗号堆砌无空间位置/朝向，图像模型出错率高；(3) 无 prompt 质量检测。**schema 3 个新 optional 字段**：`camera_position`（镜头物理位置入 [Shot]）/ `char_layout[]`（角色空间布局→分段式 [Subject]，每角色一段落含 position+orientation+action）/ `negative_additions`（逐帧反向提示）。**PromptComposer 4 项改动**（Python+TS 同步）：venue_type 条件注入（有 scene_prompt 时跳过）/ char_layout 分段式 Subject / camera_position 注入 / markdown 格式输出。**新工具 `tools/prompt_check.py`**（236 行）：6 项机械检测（C1 venue_conflict / C2 spatial_missing / C3 gaze_ambiguous / C4 scene_conflict / C5 prompt_too_long / C6 duplicate_content）。**CC session 职责边界**：`local_cc.py` _LDD_SYSTEM_CONTEXT 加「⛔ 职责边界」段，webapp CC session 仅操作 spec 内容层、禁止诊断代码。`[来源: Steve 直接指示（2026-06-04）+ 第一原理推导]` |
