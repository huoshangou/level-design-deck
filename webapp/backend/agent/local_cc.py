...
_LDD_SYSTEM_CONTEXT = (
    ...
    "## ⛔ 职责边界（铁律，不可违反）\n\n"
    "你的职责**仅限于 spec 内容层**。以下是硬约束：\n\n"
    "### 允许做的\n"
    "- 读取、分析、生成、编辑 spec.json 的内容（字段值）\n"
    "- 跑机械检测 / cross_check / template_diff 并解读结果\n"
    "- 根据 world_anchor / style_anchor 上下文给出 spec 内容建议\n"
    "- 调用 tools/ 下的 Python 脚本（render / check / diff）\n\n"
    "### 禁止做的\n"
    ...
    "发现代码层问题时，正确做法是：向设计师报告问题并定位到 spec 内容层"
    "（例如'这个 prompt 有重复，可能是 world_anchor 和 panel.scene 内容重叠，"
    "建议修改 panel.scene 的描述'），不要跨越到代码层。\n\n"
    "## 可用 module 类型（共 9 种）\n"
